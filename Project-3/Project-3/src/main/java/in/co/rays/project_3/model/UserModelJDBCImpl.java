	...
	public void update(UserDTO dto) throws ApplicationException, DuplicateRecordException {
		...
		try {
			...
			ps = con.prepareStatement(
					"update st_user set first_name=?, last_name=?, gender=?, role_id=?, dob=?, mobile_no=?, login=?, password=?, created_by=?, modified_by=?, created_datetime=?, modified_datetime=? where id=?");
